...
// 是否在剪裁区域内
export function inClipPath(point, clipPath) {
  const x = clipPath[0], y = clipPath[1], width = clipPath[2], height = clipPath[3]
  const p = [
    {x: x, y: y},
    {x: x + width, y: y},
  ...
  return isInPolygon(point, p)
}

/**
 * 判断点是否在x6图片节点内,
 * 标识X6自定义图片：data.shape='$backgroundImage' 
 * 图片节点：data?.shape='image'
 * @param {Point} point 点
 * @param {Point} points 坐标组
 * @param {number} angle 中心点旋转角度
 * @param {number} style 
 * @param {number} data x6业务数据
 */
export function isInX6ImageNode(point, points, angle, style, data) {
  if (data?.shape !== '$backgroundImage'&& data?.shape !== 'image') return
  const p = [...points, 
    {x: points[0]?.x + style.width, y: points[0]?.y},
    {x: points[0]?.x + style.width, y: points[0]?.y + style.height},
    {x: points[0]?.x, y: points[0]?.y + style.height},
  ]
  return isInPolygon(point, p, angle)
}
